...
class MockRequestBuilder: BaseRequestBuilder, RequestBuilding {
    ...
    init() {
        super.init(store: MockUserProfileStore(), config: MockSettingsConfigurable(defaults: UserDefaults(suiteName: nil)!), crypto: MockEncryptable(), uniqueStringProviding: UniqueStringProvider())
    }
